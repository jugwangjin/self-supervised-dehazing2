...
class RESIDEHazyDataset_save(torch.utils.data.Dataset):
    def __init__(self, root, mode, patch_size=128, ratio=1):
        ...
        for fn in img_set:
            dirs = fn.split('/')[:-2]
            clear_name = os.path.join(*dirs, *['clear'], fn.split('/')[-1].split('_')[0]+'.jpg')
            os.system(f'cp /{clear_name} /Bean/data/gwangjin/outdoor_gt/{clear_name.split("/")[-1]}')
...
class HSTSDataset(torch.utils.data.Dataset):
    '''
    Dataset with only RESIDE_beta test set - internet collected unpaired hazy images
    '''
    def __init__(self, root, mode='val', ratio = 0.1, patch_size=128):
        ...
        dir2 = os.path.join(root, 'test', 'HSTS', 'synthetic', 'synthetic')
    ...
    def __getitem__(self, index):
        img = Image.open(self.img_set[index]).convert("RGB")
        clear_img_name = self.img_set[index].split('/')[:-2] + ['original'] + [self.img_set[index].split('/')[-1]]
        clear_img_name = "/"+os.path.join(*clear_img_name)
        clear_img = Image.open(clear_img_name).convert("RGB")
...
class MergedDataset(torch.utils.data.Dataset):
    def __init__(self, root, mode='val', ratio = 0.1, patch_size=128):
        ...
        self.dataset = torch.utils.data.ConcatDataset([reside_standart_dataset, reside_hazy_dataset])
        print(f'Dataset built - length {len(self.dataset)}, from res_std {len(reside_standart_dataset)}, res_outd {len(reside_hazy_dataset)}')
